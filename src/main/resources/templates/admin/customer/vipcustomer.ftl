...
<script>
    $(document).ready(function () {
        ...
        $("#table_list").bootstrapTable({
            ...
            //获取数据的Servlet地址
            url: "${ctx!}/admin/customer/viplist",
            //表格显示条纹
            striped: true,
            //启动分页
            pagination: true,
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    if (value == '0')
                        return '<span class="label label-important">非会员</span>';
                ...
                formatter: function (value, row, index) {
                    ...
                    operateHtml = operateHtml + '<@shiro.hasPermission name="system:user:grant"><button class="btn btn-info btn-xs" type="button" onclick="viptocommon(\''+row.id+'\')"><i class="fa fa-arrows"></i>&nbsp;取消会员</button></@shiro.hasPermission>';
                    return operateHtml;
                }
            }]
        });
    ...
    function viptocommon(id){
        layer.confirm('确定取消会员吗?', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "${ctx!}/admin/customer/viptocommon/" + id,
                success: function(msg){
                    layer.msg(msg.message, {time: 2000},function(){
                        $('#table_list').bootstrapTable("refresh");
                        layer.close(index);
