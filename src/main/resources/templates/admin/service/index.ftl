    ...
    <script>
        $(document).ready(function () {
			...
			$("#table_list").bootstrapTable({
			    ...
			    columns: [{
                    ...
                    title: "已售",
                    field: "servicehavesaved"
                },{
                    title: "删除状态",
                    sortable: true,
                    field: "servicedeletestatus",
        ...
        function edit(id){
        	layer.open({
        	      ...
        	      shade: false,
        	      area: ['893px', '600px'],
        	      content: '${ctx!}/admin/service/edit/' + id,
        	      end: function(index){
        ...
        function del(id){
        	layer.confirm('确定删除吗?', {icon: 3, title:'提示'}, function(index){
        		$.ajax({
    	    		   type: "POST",
    	    		   dataType: "json",
    	    		   url: "${ctx!}/admin/service/delete/" + id,
    	    		   success: function(msg){
	 	   	    			layer.msg(msg.message, {time: 2000},function(){
	 	   	    				$('#table_list').bootstrapTable("refresh");
	 	   	    				layer.close(index);
